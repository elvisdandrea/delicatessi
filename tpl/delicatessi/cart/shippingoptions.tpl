{foreach from=$shipping_options key="ship" item="option"}
    <div class="shipping_option">
        <p><input type="radio" name="shipping_option" value="{$ship}" onchange="Main.quickLink('{$smarty.const.BASEDIR}cart/selshipping?{if ($order_id)}order_id={$order_id}&{/if}id={$ship}')"/>
